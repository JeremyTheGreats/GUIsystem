...
public final class MySales extends javax.swing.JFrame {
    ...
    public void displayUserTransactions() {
        ...
        // SQL JOIN to get product names from the details table
        String sql = "SELECT s.sale_id AS 'ID', sd.p_name AS 'Product Name', "
                + "sd.p_qty AS 'Qty', s.total_amount AS 'Total', s.sale_date AS 'Date' "
                + "FROM sales s "
                + "JOIN sales_details sd ON s.sale_id = sd.sales_id "
                + "WHERE s.u_id = " + currentUserId + " "
                + "ORDER BY s.sale_date DESC";

        // Assuming 'sales' is the variable name of your JTable
        con.displayData(sql, sales);
    }
    ...
    private void initComponents() {
        ...
        sales.setFont(new java.awt.Font("Tahoma", 0, 12)); // NOI18N
    ...
    private void searchActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_searchActionPerformed
        ...
        String findings = find.getText().trim();
        config con = new config();
        int currentUserId = Session.getInstance().getId();

        // Use '?' as placeholders for safety and flexibility
        String sql = "SELECT s.sale_id AS 'ID', sd.p_name AS 'Product Name', "
                + "sd.p_qty AS 'Qty', s.total_amount AS 'Total', s.sale_date AS 'Date' "
                + "FROM sales s "
                + "JOIN sales_details sd ON s.sale_id = sd.sales_id "
                + "WHERE s.u_id = ? AND (sd.p_name LIKE ? OR s.sale_id LIKE ?)";

        // Pass parameters: the user ID and the search term wrapped in '%'
        con.displayData(sql, sales,
                currentUserId,
                "%" + findings + "%",
                "%" + findings + "%");
    ...
    private void findKeyTyped(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_findKeyTyped
        ...
        // SQL with JOIN to get Product Names, filtered by current user and search text
        String sql = "SELECT s.sale_id AS 'ID', sd.p_name AS 'Product Name', "
                + "sd.p_qty AS 'Qty', s.total_amount AS 'Total', s.sale_date AS 'Date' "
                + "FROM sales s "
                + "JOIN sales_details sd ON s.sale_id = sd.sales_id "
        ...
        // Pass the parameters to your config.displayData method
        // We wrap 'findings' in % to search for any part of the name
        con.displayData(sql, sales,
                currentUserId,
                "%" + findings + "%",
                "%" + findings + "%");
    ...
    private void productMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_productMouseClicked
        SearchProduct d = new SearchProduct();
        d.setVisible(true);
        this.dispose();
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(MySales.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
